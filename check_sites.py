"""Quick site checker — tests all Shopify stores in sites/ directory.
Reports: live, dead, parked, empty, ssl_error counts + writes CSV.
"""
...
async def check_site(session, url, sem):
    async with sem:
        ...
        try:
            # Test /products.json
            async with session.get(f"{url}/products.json?limit=1", timeout=aiohttp.ClientTimeout(total=TIMEOUT)) as resp:
                ...
                if resp.status == 200:
                    try:
                        ...
                        if products:
                            ...
                            async with session.get(url, timeout=aiohttp.ClientTimeout(total=TIMEOUT)) as r2:
                                html = await r2.text()
